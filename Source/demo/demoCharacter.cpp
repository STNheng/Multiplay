...
void AdemoCharacter::CreateGameSession()
{
...
void AdemoCharacter::OnCreateSessionComplete(FName SessionName, bool bSuccess)
{
	if (bSuccess)
	{
		if (GEngine)
...
void AdemoCharacter::JoinGameSession()
{
	//Find Game Session
	if (!OnlineSessionInterface.IsValid())
	{
...
void AdemoCharacter::OnFindSessionsComplete(bool bSuccess)
{
	for (auto Result : SessionSearch->SearchResults)
	{
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1,15.f,FColor::Red,FString::Printf(TEXT("Id: %s, User: %s"),*Id,*User));
		}
	}
}
